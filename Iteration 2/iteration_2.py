...
def signup():
    ...
    def submit():
        ...
        # --- Username validation ---
        if len(username) < 6:
            messagebox.showerror("Signup Error", "Username must be at least 6 characters long.")       #username length check
            return

        # --- Password validation ---
        if len(password) < 6:
            messagebox.showerror("Signup Error", "Password must be at least 6 characters long.")     #password length check
            return
        ...
        try:
            age = int(age)
            if age < 13:
                messagebox.showerror("Signup Error", "You must be at least 13 years old.")          #age restriction
                return
        except ValueError:
            messagebox.showerror("Signup Error", "Invalid age input.")                    #non-integer age input
            return
...
def login(app):
    ...
    def submit():
        ...
        try:
            ...
            messagebox.showerror("Error", "No user file found. Please sign up first.")         #no users.txt file
            window.destroy()
            return
